...
### Critical / Major
- [ ] #6 Fix scanner cron crash when DB is down — `updateScanJob(job.id)` throws TypeError if `createScanJob()` fails (`job` is undefined). Guard catch block with `if (job)`.
...
### Minor
- [ ] #8 Add retry limit to 429 recursion in GitHub client — no max retries, persistent 429s cause infinite recursion. Add counter with max 3.
